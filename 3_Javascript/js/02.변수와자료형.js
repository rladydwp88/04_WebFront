...
// 변수에 대입한 값 변경하기
number1 = 300; // number1에 300 재대입(기존 10값에 덮어씀)
...
// 형변환 없이 출력되어 문자열 따로, 다른 데이터 타입 따로 독립되어 출력
...
// let과 const로 선언된 변수는
// 변수가 선언 된 블록(중괄호 {}) 내부에서만 유효하다는 것을 의미함.
...
// 함수 레벨 스코프 (var)
// var 키워드로 선언 된 변수는 함수 내 어디서든 접근할 수 있음.
...
// 함수 안에서는 어떻게 될끼?
function example() { // 이렇게 함수 형태로 만들어진 코드 블록은 함수레벨 scope라고 함
...
function example2() {
  ...
  if(true) { // 블록문(if, for, while 등) 내에서 선언된 var 변수는 일반 블록 무시함
    var x = 10; // x 는 블록 내부에서 선언되었지만 함수레벨 스코프를 가진 var 변수
  }

  console.log(x); // 블록레벨에서 변수에 접근
...
console.log(age, height, eyesight);
